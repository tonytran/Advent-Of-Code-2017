in_ = '''4347	3350	196	162	233	4932	4419	3485	4509	4287	4433	4033	207	3682	2193	4223
...
def parse_input(input):
    ...
    for a in arrs_:
        ...
        if '\t' in a:
            ...
            li = a.split(' ')
        list_of_numbers = [int(x) for x in li]
        arrs.append(list_of_numbers)
    return arrs
def get_check_sum_divide(arr_of_arrs):
    _sum = 0
    print(arr_of_arrs)
        ...
        for i in range(len(arr)) :
            for n in range(i+1, len(arr)):
                ...
                #    print(arr[n], arr[i])
                    _sum += (arr[n] // arr[i])
    return _sum
def get_check_sum(arr_of_arrs):
    _sum = 0
    for arr in arr_of_arrs:
        _sum += (max(arr) - min(arr))
    return _sum
def main():
    input_part_one = parse_input(
    '''5 1 9 5
    7 5 3
    2 4 6 8'''
    )
    input_part_two = parse_input(
    '''5 9 2 8
    9 4 7 3
    3 8 6 5''')
    assert get_check_sum(input_part_one) == 18
    assert get_check_sum_divide(input_part_two) == 9
    #print(get_check_sum_divide(parse_input(in_)))
main()
